...
export async function initUserDataDB() {
  ...
  `);
    ...
    if (!setTypesBackfillDone) {
      ...
      for (const tmpl of setTypeTemplates) {
        ...
        for (const { id: ceId } of matchingIds) {
          for (let i = 0; i < planSets.length; i++) {
            const setNum = i + 1;
  ...
  `);
    ...
    if (!trackedExercisesResult.some((c) => c.name === "sort_order")) {
      await db.withExclusiveTransactionAsync(async (txn) => {
        ...
        await txn.execAsync(`UPDATE tracked_exercises SET sort_order = rowid`);
      });
    }
  ...
  `);
    ...
    // a no-op. `notes` needs none: its UNIQUE (type, reference_id,
    // secondary_reference_id) constraint is backed by an index whose left
    // prefix already serves (type, reference_id) lookups.
    await db.execAsync(`
    ...
    CREATE INDEX IF NOT EXISTS idx_uwe_workout    ON user_workout_exercises(workout_id);
